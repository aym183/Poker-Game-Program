...
def Check_Four_Of_A_Kind(array):
    if not Check_Flush(array) and not Check_Straight(array): # The operation is conducted further only if it's not a Flush or Straight
        ...
        count2 = []
        for i in array: 
            new2.append(Ranks.get(i[0])) #looped through the input array, take the integer from the string and added its rank value from the dictionary to another array i.e new2
            x = np.array(new2)
            for i in np.unique(x): # Use of numpy to take only the unique values from new2
      
                if new2.count(i)==4: # .count to check how many occurences of each value in the hand. If 4, then it is added to a new list
            
                    if i not in count2: # Added if not already in the list 
                        count2.append(i)
...
def Check_Full_House(array):
    new=[]
    for i in array: #looped through the input array, take the integer from the string and added its rank value from the dictionary to another array i.e new
        new.append(Ranks.get(i[0]))
    ...
    for i in np.unique(x): # Use of numpy to take only the unique values from new
        ...
        if new.count(i)==2 or new.count(i)==3: #  .count to check how many occurences of each value in the hand. If 2 or 3, then it is added to a new list. I.E count2
            count2.append(i)
        
    
    
    if len(count2)==2: # The operation proceeds then and checks if length of count2 is 2 because if it is a full house, it will consists of only 2 different ranks. Eg -> 5 and 6
        
        for i in count2:
            if new.count(i)==2: # If .count of any of the 2 elements is 2, it is added to the first position of a new list 
                count3.insert(0,i)
            elif new.count(i)==3:
                count3.insert(1,i) # If .count of any of the 2 elements is 3, it is added to the second position of a new list 
    ...
    if len(count3)==2:
        if new.count(count3[0])==2 and new.count(count3[1])==3:
            #print(count3)
            return True
...
def Three_Of_A_Kind(array):
    ...
    if Check_Full_House(array) and not Check_Four_Of_A_Kind(array): # If hand is a Full House or Four Of A Kind, it will return False. This is because both cases will have 3 occurences of some rank 
        ...
        count2 = []
        for i in array:
            new2.append(Ranks.get(i[0]))
            x = np.array(new2)
            for i in np.unique(new2):
      
                if new2.count(i)==3: # THe rest is similar to the Four Of A Kind operation, only the .count is specified to verify if value is 3 instead of 4
    ...
    if not Check_Full_House(array): # It can be a full house as the elements of the 2nd pair can consist of 3 occurences hence returned false if it is a full house
        ...
        for i in array:
            ...
            for i in np.unique(new):
                ...
                if new.count(i)==2: # If.count of that element is 2, means it is a pair and is added to a new list
            
                    if i not in count:
                        count.append(i)
            
    
        if len(count)==2: # If length of new list is 2, means there are 2 pairs
...
def Check_Pair(array):
    ...
    if not Check_Full_House(array) and not Check_Two_Pairs(array):# It can be a full house as the elements of the 2nd pair can consist of 3 occurences or Two Pairs, hence returned false if it is a full house or Two Pairs
        ...
        if len(count)==1: # If length of new list is 1, means there is 1 pair
...
def Main():
        ...
        best_Hand = []
        Individual = []
        #randomize_Cards_dict = list(Cards.values())
        #randomize_Suits_dict = list(Suits.values())
